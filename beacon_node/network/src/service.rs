    ...
    for (client, scores) in scores_per_client.into_iter() {
        ...
        if len > 0 {
            ...
            for &score in &scores {
                ...
                if score < -4000.0 { //TODO not hardcode
                    below_gossip_threshold += 1;
                }
                if score < -8000.0 { //TODO not hardcode
                    below_publish_threshold += 1;
                }
                if score < -16000.0 { //TODO not hardcode
            ...
            metrics::set_gauge_entry(&metrics::SCORES_BELOW_ZERO_PER_CLIENT, c,
                                     below0 as f64 / count);
            metrics::set_gauge_entry(&metrics::SCORES_BELOW_GOSSIP_THRESHOLD_PER_CLIENT, c,
                                     below_gossip_threshold as f64 / count);
            metrics::set_gauge_entry(&metrics::SCORES_BELOW_PUBLISH_THRESHOLD_PER_CLIENT, c,
                                     below_publish_threshold as f64 / count);
            metrics::set_gauge_entry(&metrics::SCORES_BELOW_GREYLIST_THRESHOLD_PER_CLIENT, c,
